...
async def get_posts(db: AsyncConnection = Depends(get_db)) -> list[PostOut]:
    '''Retrieve all posts.'''
    try:
        query = text('SELECT id, slug, title, content, tags FROM posts WHERE is_published = true ORDER BY created_at DESC')
...
async def get_post(slug: str, db: AsyncConnection = Depends(get_db)) -> PostOut:
    ...
    try:
        query = text('SELECT id, slug, title, content, tags FROM posts WHERE slug = :slug AND is_published = true LIMIT 1')
